...
public class CollectionMapper implements ComplexMapperHelper
{
	        ...
	        throws MapperException
	{
		...
		Collection lCollObj = (Collection) aPojo;
		
		try
		{
			// If the object passed in is not null, we can just clear that
			// collection and
			// start using it.
			if (lCollObj != null)
			{
		...
		} else if (aTypes.length == 1)
		{
			...
			for (JSONValue lVal : aObject.getValue())
			{
				...
				{
					ParameterizedType pt = (ParameterizedType) aTypes[0];
					Class rawType = (Class) pt.getRawType();
					
					lCollObj.add(JSONMapper.toJava(lVal, null, pt));
				} else
				{
					Class clazz = (Class) aTypes[0];
					lCollObj.add(JSONMapper.toJava(lVal, null, clazz));
				}
			}
		} else
		{
			...
			// its contents.
			throw new MapperException("CollectionMapper cannot map: " +
			        aValue.getClass().getName() + " Too many arguments");
		}
